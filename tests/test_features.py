...
from sandwitches.models import Recipe, Rating, Tag
...
class FeatureTests(TestCase):
    def setUp(self):
        ...
        self.superuser = User.objects.create_superuser("admin", "admin@example.com", "strongpassword123")
        self.recipe = Recipe.objects.create(title="Test Recipe", instructions="Instructions", ingredients="Ingredients", uploaded_by=self.user1)
    ...
    def test_user_rating_and_comment_on_detail_page(self):
        ...
        response = self.client.post(reverse('recipe_rate', args=[self.recipe.pk]), {
            'score': 8.5,
    ...
    def test_multiple_ratings_and_comments_on_detail_page(self):
        # User1 rates
        self.client.login(username="user1", password="password")
        self.client.post(reverse('recipe_rate', args=[self.recipe.pk]), {
            'score': 8.0,
