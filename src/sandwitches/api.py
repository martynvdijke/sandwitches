...
def update_settings(request, payload: SettingSchema):
    ...
    logger.info(f"User {request.user.username} is updating site settings")
...
def me(request):
    ...
    logger.debug(f"User {request.user.username} requested their own profile")
    return request.user


@api.get("v1/users", response=list[UserSchema])
def users(request):
    logger.debug("Listing all users via API")
    return User.objects.all()


@api.get("v1/recipes", response=list[RecipeSchema])
def get_recipes(request):
    logger.debug("Listing all recipes via API")
...
def scale_recipe_ingredients(request, recipe_id: int, target_servings: int):
    ...
    for line in ingredient_lines:
        try:
            ...
            logger.warning(f"Failed to scale ingredient line '{line}': {e}")
