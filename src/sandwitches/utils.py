...
COMMON_UNITS = {
    "cup", "cups", "oz", "ounce", "ounces", "g", "gram", "grams", "kg", "kilogram", "kilograms",
    ...
    "slice", "slices", "clove", "cloves", "large", "medium", "small", "can", "cans", "package",
    "packages", "piece", "pieces", "dash", "dashes", "sprig", "sprigs", "to taste"
}
...
def parse_ingredient_line(line):
    ...
    # - `1.5` (decimal)
    # - `1` (integer)
    quantity_match = re.match(r'^\s*(\d+\s+\d/\d|\d/\d|\d+\.?\d*|\.\d+)\s*(.*)', original_line)
    ...
    for common_unit in sorted(list(COMMON_UNITS), key=len, reverse=True):
        ...
        unit_regex = r"^\s*" + re.escape(common_unit) + r"(\b|\s|$)"
    ...
    name = remaining_line if remaining_line else original_line if quantity is None and unit is None else ""
    ...
    if quantity is None and unit is None:
        ...
        if unit and original_line.lower().startswith(f"{quantity_str.lower() if quantity_str else ''} {unit.lower()}".strip()):
...
def scale_ingredient(parsed_ingredient, current_servings, target_servings):
    ...
    return {
        ...
        'original_line': parsed_ingredient['original_line'], # Keep original line reference
    }
...
def format_scaled_ingredient(scaled_ingredient):
    ...
    if quantity is None:
        return scaled_ingredient['original_line'] # Fallback to original if quantity was not parsed
